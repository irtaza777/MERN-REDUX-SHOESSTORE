...
const Products = () => {
    ...
    return (
        <div className="container mx-auto py-10 flex flex-col">
            ...
            <button onClick={toggleSidebar} className="mb-4  text-black text-3xl">
                        ...
                        {filteredProducts.map((product) => (
                            <div key={product.id} className="bg-slate-100 shadow rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                                ...
                                <div className="p-4 text-center ">
                                    ...
                                    <div className="mt-2">
                                        ...
                                        <div className="flex space-x-2 mt-1">
                                            {product.colors.map((color) => (
                                                <div
                                                    key={color.id}
                                                    className={`w-6 h-6 rounded-full cursor-pointer ${selectedOptions[product.id]?.color === color.color.name ? 'border-2 border-black' : ''
                                                        }`}
                                                    style={{ backgroundColor: color.color.name.toLowerCase() }}
                                                    title={color.color.name}
                                                    onClick={() => handleColorSelection(product.id, color.color.name)}
                                                ></div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-2">
                                        ...
                                        <div className="flex space-x-2 mt-1">
                                            {product.sizes.map((size) => (
                                                <span
                                                    key={size.id}
                                                    className={`px-2 py-1 bg-gray-200 text-sm rounded-md cursor-pointer ${selectedOptions[product.id]?.size === size.size ? 'bg-gray-400' : ''
                                                        }`}
                                                    onClick={() => handleSizeSelection(product.id, size.size)}
                                                >
                                                    {size.size}
                                                </span>
                                            ))}
                                    ...
                                    {/* Add to Cart Button */}
                                    <button
                                        className="mt-4 p-4 bg-orange-600 text-white py-2 rounded hover:bg-red-950 transition duration-200"
